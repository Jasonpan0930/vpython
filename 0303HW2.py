...
scene = canvas(width=600, height=600,x=0, y=0, center = vector(0,0,0), background=vector(0,0,0)) #設定畫面
floor = box(length=45, height=0.01, width=10, texture=textures.wood)  	#畫地板
blue_ball = sphere(radius = size, color=color.blue, make_trail= True) 	#畫球
green_ball = sphere(radius = size, color=color.green, make_trail= True) 	#畫球
...
while blue_ball.pos.y >= 0:    #模擬直到球落地 即y=0
    ...
    blue_ball.a2=k*blue_ball.v
    blue_ball.v = blue_ball.v + blue_ball.a*dt+blue_ball.a2*dt          #球的末速度 = 前一刻速度 + 加速度*時間間隔
    blue_ball.pos = blue_ball.pos + blue_ball.v * dt    #球的末位置 = 前一刻位置 + 速度*時間間隔


    if(abs(blue_ball.v.y-0)<=0.1 and t>0.2 and blue_ball.pos.y>0):
        y1=blue_ball.pos.y
t1=t
d1=blue_ball.pos.x
...
while green_ball.pos.y >= 0:    #模擬直到球落地 即y=0
    ...
    green_ball.a2=k*green_ball.v
    green_ball.v = green_ball.v + green_ball.a*dt + green_ball.a2*dt          #球的末速度 = 前一刻速度 + 加速度*時間間隔

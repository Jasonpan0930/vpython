...
while t < 10:
    ...
    for i in range(7):
        ...
        if light[i].pos.y>0 and switch[i]==0:    #第i道光線抵達界面                
            ...
            if n1*sin(theta_i[i])>n2:     #全反射條件
                light[i].v.y *=-1   #改變光線行進方向
                theta_r.append(0)
            else :    #折射定律
                theta_r.append(asin(n1*sin(theta_i[i])/n2))    #利用折射定律配合反三角函數求出各個折射角並放入theta_r的list中
                light[i].v = n1*vec (sin(theta_r[i]),cos(theta_r[i]),0)    #改變光線行進方向
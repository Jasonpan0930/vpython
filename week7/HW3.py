...
for i in range(m):
    n.append(k*delta_y*i+1)       #切1000層，由下而上的各層折射率
...
while t < 25:
    ...
    if light.v.y < 0 and light.pos.y // delta_y != (light.pos.y + light.v.y*dt) // delta_y :    #光線下行時通過各介面的判斷
        ...
        if first_time == 0:                 #判斷第一次的折射
            ...
            theta_r = asin((n[999]*sin(theta_i)/n[i]))    #利用折射定律配合反三角函數求出折射角
            light.v = vec (sin(theta_r),-cos(theta_r),0)            #改變光線行進方向
            first_time = 1
            
        else:
            theta_i = theta_r    #每一次入射角為上一次的折射角
            if n[i+1]*sin(theta_i)>n[i]:      #全反射條件
                print("here")
                light.v.y *= -1    #發生全反射時，光線前進方向如何調整?               
            else:
                theta_r = asin(n[i+1]*sin(theta_i)/n[i])        #利用折射定律配合反三角函數求出折射角
                light.v = n[i]/n[i+1]*vec (sin(theta_r),-cos(theta_r),0)   #改變光線行進方向
